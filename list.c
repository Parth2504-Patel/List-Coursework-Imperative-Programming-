...
list *newList(item e){
  list *xs = (list *) malloc(sizeof(list));
  node *sentinel = (node *) malloc(sizeof(node));
  xs->none = sentinel;
  xs->current = sentinel;
  *sentinel = (node){xs -> none, e, xs -> current};
  return xs;
}
...
void first(list *xs){
  ...
  else xs->current = xs->none->next;
}

void last(list *xs){
  if (xs->none->back == xs->none);
  else xs->current = xs->none->back;
}
...
bool before(list *xs){
  if (none(xs) == true) return false;
  else if (xs->current == xs->none->next){
...
void insertAfter(list *xs, item x){
  ...
  *elem = (node){xs->current, x, xs->current->next};
  Next->back = elem;
  Curr->next =elem;
  xs->current =elem;
}
...
void insertBefore(list *xs, item x){
  node *elem = (node *)malloc(sizeof(node));
  node *Curr = xs->current;
  node *Back = xs->current->back;
  *elem = (node){xs->current->back , x, xs->current};
  Back->next = elem;
  Curr->back = elem;
  xs->current = elem;
}
